...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        # 🛡️ 防止空关系对导致 union_feature 报错
        if not rel_pair_idxs or all(len(pairs) == 0 for pairs in rel_pair_idxs):
            if logger:
                logger.info("⚠️ Skipping image with no valid relationship pairs.")
            empty_tensor = roi_features.new_zeros(roi_features.size(0), self.predictor.ctx_compress.out_channels)
            return roi_features, proposals, {"loss_rel": torch.tensor(0.0).to(roi_features.device),
                                                 "loss_refine_obj": torch.tensor(0.0).to(roi_features.device)}
