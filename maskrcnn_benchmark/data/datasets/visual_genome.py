...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                filter_empty_rels=True, num_im=-1, num_val_im=None,
        ...
        # 从配置文件读取 num_val_im，默认为 0
        from maskrcnn_benchmark.config import cfg
        num_val_im = cfg.DATASETS.get('NUM_VAL_IMAGES', 0) if num_val_im is None else num_val_im
        print(f"num_val_im set to: {num_val_im}")
        ...
        if self.custom_eval:
            ...
            print(f"Split mask sum before filtering: {self.split_mask.sum()}, Split mask length: {len(self.split_mask)}")
            print(f"First 10 split_mask values: {self.split_mask[:10]}")
            print(f"Number of filenames before filtering: {len(self.filenames)}")
            #print("Filenames length:", len(self.filenames))
            #print("Filenames:", self.filenames[:10])  # 打印前10个文件名
            #print("Split mask length:", len(self.split_mask))
            #print("Split mask indices:", np.where(self.split_mask)[0])
            self.filenames = [self.filenames[i] for i in np.where(self.split_mask)[0]]
            self.img_info = [self.img_info[i] for i in np.where(self.split_mask)[0]]
            print(f"Number of images after filtering: {len(self.filenames)}")
            print(f"First 10 filenames after filtering: {self.filenames[:10]}")
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        print(f"Image {index}: raw boxes: {box}")
        box = torch.from_numpy(box).reshape(-1, 4)  # guard against no boxes
        invalid = (box[:, 2] <= box[:, 0]) | (box[:, 3] <= box[:, 1])
        if invalid.any():
            print(f"Image {index}: {invalid.sum()} invalid boxes: {box[invalid]}")
            # Fix invalid boxes
            box[:, 2] = torch.max(box[:, 2], box[:, 0] + 1)
            box[:, 3] = torch.max(box[:, 3], box[:, 1] + 1)
        if flip_img:
            new_xmin = w - box[:,2]
            new_xmax = w - box[:,0]
            box[:,0] = new_xmin
        ...
        print(f"Image {index}: BoxList: {target.bbox}")
        ...
        print(f"Image {index}: relations: {relation}")
        ...
        if evaluation:
            ...
            print(f"Image {index}: BoxList after clip: {target.bbox}")
            if len(target) == 0:
                print(f"Warning: Image {index} has no valid boxes after clipping")
            return target
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    ...
    print(f"Initial split_mask sum: {split_mask.sum()}, length: {len(split_mask)}")
    print(f"First 10 split_mask values after split selection: {split_mask[:10]}")
    print(f"data_split dtype: {data_split.dtype}, shape: {data_split.shape}")
    # Filter out images without bounding boxes
    split_mask &= roi_h5['img_to_first_box'][:] >= 0
    print(f"Split_mask sum after box filter: {split_mask.sum()}")
    if filter_empty_rels:
        split_mask &= roi_h5['img_to_first_rel'][:] >= 0
        print(f"Split_mask sum after rel filter: {split_mask.sum()}")

    image_index = np.where(split_mask)[0]
    print(f"Image indices: {image_index}")
    print(f"image_index dtype: {image_index.dtype}, shape: {image_index.shape}")
    if num_im > -1:
        image_index = image_index[:num_im]
        print(f"Image indices after num_im: {image_index}")
    if num_val_im > 0:
        if split == 'val':
            image_index = image_index[:num_val_im]
            print(f"Image indices after num_val_im (val): {image_index}")
        elif split == 'train':
            if num_val_im >= len(image_index):
                print(f"Warning: num_val_im ({num_val_im}) >= len(image_index) ({len(image_index)}), setting num_val_im to 0")
                num_val_im = 0
            else:
                image_index = image_index[num_val_im:]
                print(f"Image indices after num_val_im (train): {image_index}")
    else:
        print(f"num_val_im is 0, no validation split applied")
            #image_index = image_index[num_val_im:]
        #print(f"Image indices after num_val_im: {image_index}")


    split_mask = np.zeros_like(data_split).astype(bool)
    print(f"split_mask dtype before assignment: {split_mask.dtype}, shape: {split_mask.shape}")
    for idx in image_index:
        split_mask[idx] = True
    #split_mask[image_index] = True
    print(f"Split_mask sum after image_index: {split_mask.sum()}")
    print(f"First 10 split_mask values after image_index: {split_mask[:10]}")
    # Get box information
    ...
    for i in range(len(image_index)):
        ...
        if filter_non_overlap:
            ...
            print(f"Image {image_index[i]}: num_rels={len(rels)}, num_overlapping={inc.size}, boxes={boxes_i.shape}")
            if inc.size > 0:
                rels = rels[inc]
            else:
                split_mask[image_index[i]] = 0
                print(f"Image {image_index[i]} filtered due to no overlapping boxes")
                continue
    ...
    print(f"Final split_mask sum: {split_mask.sum()}, length: {len(split_mask)}")
    print(f"First 10 split_mask values in load_graphs: {split_mask[:10]}")
