...
class RelationFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.rect_conv.eval()  # Prevent AMP-related BN issues

    def chunked_rect_conv(self, rect_inputs, chunk_size=256):
        outputs = []
        for i in range(0, rect_inputs.shape[0], chunk_size):
            chunk = rect_inputs[i:i+chunk_size]
            outputs.append(self.rect_conv(chunk))
    ...
    def forward(self, x, proposals, rel_pair_idxs=None):
        ...
        for proposal, rel_pair_idx in zip(proposals, rel_pair_idxs):
            ...
            head_rect = head_rect.unsqueeze(1)
            tail_rect = tail_rect.unsqueeze(1)
            rect_input = torch.cat((head_rect, tail_rect), dim=1)
            rect_inputs.append(rect_input)

        rect_inputs = torch.cat(rect_inputs, dim=0)

        if (
            rect_inputs.ndim != 4 or
            rect_inputs.shape[0] == 0 or
            rect_inputs.shape[2] < 3 or
            rect_inputs.shape[3] < 3 or
            torch.isnan(rect_inputs).any() or
            torch.isinf(rect_inputs).any() or
            rect_inputs.abs().max() > 1e4
        ):
            return rect_inputs.new_zeros((0, self.out_channels), dtype=rect_inputs.dtype, device=rect_inputs.device)

        rect_features = self.chunked_rect_conv(rect_inputs)
